...
async def cb_about(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💬 Предложения и пожелания", callback_data="about_feedback")],
...
async def cb_about_donate(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    rows = [[InlineKeyboardButton(text=label, callback_data=f"donate_{kop}")] for kop, label in DONATE_AMOUNTS]
